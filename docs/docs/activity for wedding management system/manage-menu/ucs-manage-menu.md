...
| 2    |                                       | Truy vấn danh sách món ăn: <br>`SELECT DishId, DishName, UnitPrice, Note` <br>`FROM Dish` <br>`ORDER BY DishName`                                                                            |
...
| 6    |                                       | Truy vấn với WHERE tương ứng: <br>- Theo tên món ăn (`DishName` LIKE N'%keyword%') <br>- Theo khoảng giá (`UnitPrice` >= @Min AND `UnitPrice` <= @Max) <br>`... WHERE ... ORDER BY DishName` |
| 7    |                                       | Hiển thị kết quả theo tiêu chí tìm kiếm/lọc                                                                                                                                                  |
| 8    | Chọn món ăn muốn xem chi tiết         |                                                                                                                                                                                              |
| 9    |                                       | Truy vấn thông tin chi tiết: <br>`SELECT m.*, (SELECT COUNT(*) FROM Menu t WHERE t.DishId = m.DishId) AS SoLuongSuDung` <br>`FROM Dish m` <br>`WHERE m.DishId = @DishId`                     |
...
- Thêm index trên `Dish.DishName`, `Dish.UnitPrice` để tăng tốc tìm kiếm
...
- Tạo bản ghi `Dish` mới với thông tin đầy đủ
...
| 6    |                                | Kiểm tra tên món ăn chưa tồn tại: <br>`SELECT COUNT(*) FROM Dish WHERE DishName = @DishName`                                                |
...
| 8    |                                | **Nếu hợp lệ:** Thêm món ăn vào DB: <br>`INSERT INTO Dish (DishName, UnitPrice, Note)` <br>`VALUES (@DishName, @UnitPrice, @Note)`          |
...
- `DishName` phải UNIQUE (đã có constraint trong DB)
- `DonGia` phải > 0
...
| 4    |                                  | Truy vấn thông tin món ăn: <br>`SELECT DishId, DishName, UnitPrice, Note` <br>`FROM Dish` <br>`WHERE DishId = @DishId`                                                                                         |
| 5    |                                  | Hiển thị form chỉnh sửa với dữ liệu hiện tại                                                                                                                                                                   |
| 6    | Chỉnh sửa thông tin món ăn       |                                                                                                                                                                                                                |
| 7    | Click nút "Lưu"                  |                                                                                                                                                                                                                |
| 8    |                                  | Kiểm tra dữ liệu hợp lệ: <br>- Tên món ăn không rỗng <br>- Đơn giá > 0 <br>- Nếu tên thay đổi, kiểm tra tên mới chưa tồn tại: <br>`SELECT COUNT(*) FROM Dish WHERE DishName = @DishName AND DishId <> @DishId` |
| 9    |                                  | **Nếu dữ liệu không hợp lệ hoặc tên bị trùng:** Hiển thị thông báo lỗi, quay lại bước 6                                                                                                                        |
| 10   |                                  | **Nếu hợp lệ:** Cập nhật món ăn: <br>`UPDATE Dish` <br>`SET DishName = @DishName, UnitPrice = @UnitPrice, Note = @Note` <br>`WHERE DishId = @DishId`                                                           |
...
- Không cho phép thay đổi `DishId` (khóa chính)
...
| 4    | Click nút "Xóa"             |                                                                                                                                                                        |
| 5    |                             | Kiểm tra món ăn có đang được sử dụng không: <br>`SELECT COUNT(*) AS SoLuongThamChieu` <br>`FROM Menu` <br>`WHERE DishId = @DishId`                                     |
...
| 7    |                             | **Nếu SoLuongThamChieu = 0:** Hiển thị dialog xác nhận: "Bạn có chắc muốn xóa món ăn [DishName]?"                                                                      |
| 8    | Click "Xác nhận" hoặc "Hủy" |                                                                                                                                                                        |
| 9    |                             | **Nếu click "Hủy":** Đóng dialog xác nhận → Kết thúc                                                                                                                   |
| 10   |                             | **Nếu click "Xác nhận":** Xóa món ăn: <br>`DELETE FROM Dish WHERE DishId = @DishId`                                                                                    |
...
- Chỉ kiểm tra tham chiếu từ bảng `Menu`
...
| 6    |                                 | Truy vấn dữ liệu món ăn với bộ lọc hiện tại: <br>`SELECT m.DishId, m.DishName, m.UnitPrice, m.Note,` <br>`(SELECT COUNT(*) FROM Menu t WHERE t.DishId = m.DishId) AS SoLuongSuDung` <br>`FROM Dish m` <br>`WHERE ... ORDER BY ...` |
...
- **Bảng liên quan:** `Dish`, `Menu`, `Booking`
...
- **Xóa món ăn:** Chỉ kiểm tra tham chiếu từ `Menu`, không có xóa mềm
