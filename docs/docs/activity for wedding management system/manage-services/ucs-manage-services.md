...
| 2    |                                       | Truy vấn danh sách dịch vụ: <br>`SELECT ServiceId, ServiceName, UnitPrice, Note` <br>`FROM Service` <br>`ORDER BY ServiceName`                                                                      |
...
| 6    |                                       | Truy vấn với WHERE tương ứng: <br>- Theo tên dịch vụ (`ServiceName` LIKE N'%keyword%') <br>- Theo khoảng giá (`UnitPrice` >= @Min AND `UnitPrice` <= @Max) <br>`... WHERE ... ORDER BY ServiceName` |
| 7    |                                       | Hiển thị kết quả theo tiêu chí tìm kiếm/lọc                                                                                                                                                         |
| 8    | Chọn dịch vụ muốn xem chi tiết        |                                                                                                                                                                                                     |
| 9    |                                       | Truy vấn thông tin chi tiết: <br>`SELECT d.*, (SELECT COUNT(*) FROM ServiceDetail c WHERE c.ServiceId = d.ServiceId) AS SoLuongSuDung` <br>`FROM Service d` <br>`WHERE d.ServiceId = @ServiceId`    |
...
- Thêm index trên `Service.ServiceName`, `Service.UnitPrice` để tăng tốc tìm kiếm
...
- Tạo bản ghi `Service` mới với thông tin đầy đủ
...
| 6    |                                 | Kiểm tra tên dịch vụ chưa tồn tại: <br>`SELECT COUNT(*) FROM Service WHERE ServiceName = @ServiceName`                                       |
...
| 8    |                                 | **Nếu hợp lệ:** Thêm dịch vụ vào DB: <br>`INSERT INTO Service (ServiceName, UnitPrice, Note)` <br>`VALUES (@ServiceName, @UnitPrice, @Note)` |
...
- `ServiceName` phải UNIQUE (đã có constraint trong DB)
- `DonGia` phải > 0
...
| 4    |                                   | Truy vấn thông tin dịch vụ: <br>`SELECT ServiceId, ServiceName, UnitPrice, Note` <br>`FROM Service` <br>`WHERE ServiceId = @ServiceId`                                                                                         |
| 5    |                                   | Hiển thị form chỉnh sửa với dữ liệu hiện tại                                                                                                                                                                                   |
| 6    | Chỉnh sửa thông tin dịch vụ       |                                                                                                                                                                                                                                |
| 7    | Click nút "Lưu"                   |                                                                                                                                                                                                                                |
| 8    |                                   | Kiểm tra dữ liệu hợp lệ: <br>- Tên dịch vụ không rỗng <br>- Đơn giá > 0 <br>- Nếu tên thay đổi, kiểm tra tên mới chưa tồn tại: <br>`SELECT COUNT(*) FROM Service WHERE ServiceName = @ServiceName AND ServiceId <> @ServiceId` |
| 9    |                                   | **Nếu dữ liệu không hợp lệ hoặc tên bị trùng:** Hiển thị thông báo lỗi, quay lại bước 6                                                                                                                                        |
| 10   |                                   | **Nếu hợp lệ:** Cập nhật dịch vụ: <br>`UPDATE Service` <br>`SET ServiceName = @ServiceName, UnitPrice = @UnitPrice, Note = @Note` <br>`WHERE ServiceId = @ServiceId`                                                           |
