...
Gồm 6 use case chính, tương ứng với các sơ đồ activity trong thư mục này:
...
- Hiển thị danh sách đặt chỗ theo tiêu chí lọc (nếu có) với phân trang/sắp xếp
...
| 2    |                                | Truy vấn danh sách đặt chỗ kèm thông tin liên quan: <br>`SELECT b.*, u.full_name AS customer_name, t.departure_date, t.return_date, r.name AS route_name, i.payment_status` <br>`FROM Tour_Booking b` <br>`JOIN User u ON b.user_id = u.id` <br>`JOIN Trip t ON b.trip_id = t.id` <br>`JOIN Route r ON t.route_id = r.id` <br>`LEFT JOIN Invoice i ON i.booking_id = b.id` <br>`ORDER BY b.id DESC` |
| 3    |                                | Hiển thị danh sách với các cột: mã booking, khách hàng, tuyến/chuyến, ngày khởi hành/ngày về, seats_booked, total_price, status, payment_status, nút hành động (Xem chi tiết, Xem hóa đơn, Chỉnh sửa, Xóa)                                                                                                                                                                                          |
| 4    | Nhập tiêu chí lọc              |                                                                                                                                                                                                                                                                                                                                                                                                     |
| 5    | Click "Áp dụng"                |                                                                                                                                                                                                                                                                                                                                                                                                     |
| 6    |                                | Truy vấn với WHERE tương ứng: <br>- Theo trạng thái booking (PENDING/CONFIRMED/CANCELED/COMPLETED) <br>- Theo payment_status (UNPAID/PAID/REFUNDED) <br>- Theo khách hàng (email/phone/user_id) <br>- Theo tuyến/chuyến (route_id/trip_id) <br>- Theo khoảng ngày khởi hành <br>- Theo khoảng tổng tiền <br>`... WHERE ... ORDER BY ... LIMIT ... OFFSET ...`                                       |
| 7    |                                | Hiển thị kết quả theo tiêu chí lọc                                                                                                                                                                                                                                                                                                                                                                  |
| 8    | Xem kết quả                    |                                                                                                                                                                                                                                                                                                                                                                                                     |

### Luồng sự kiện phụ

- 2a. Không có đặt chỗ: hiển thị thông báo "Chưa có đặt chỗ nào"
- 6a. Không có kết quả phù hợp: hiển thị "Không tìm thấy đặt chỗ phù hợp với tiêu chí lọc" và cho phép xóa/thay đổi bộ lọc

### Ràng buộc nghiệp vụ/SQL gợi ý

- Thêm chỉ số (index) trên các cột lọc phổ biến: `booking.status`, `invoice.payment_status`, `trip.departure_date`, `booking.user_id`
...
Xóa hẳn một đặt chỗ khỏi hệ thống (chỉ áp dụng khi chưa thanh toán và chưa qua hạn chốt), đồng bộ số ghế trên Trip và xóa hóa đơn liên quan.
...
- Booking tồn tại
- `Tour_Booking.status = 'PENDING'` (khuyến nghị) và `Invoice.payment_status = 'UNPAID'`
- Chưa qua hạn chốt (ví dụ: ≥ 3 ngày trước `departure_date`)

### Điều kiện hậu

- Xóa các bản ghi liên quan: Booking_Traveler, Tour_Booking_Detail, Invoice, và Tour_Booking; cập nhật `Trip.booked_seats` giảm tương ứng
...
| 2    |                                     | Kiểm tra điều kiện xóa (trạng thái booking, payment_status, hạn chốt)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 3    | Xác nhận xóa                        |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 4    |                                     | Bắt đầu transaction: <br>1) Khóa Trip và Booking: <br>`SELECT id, booked_seats FROM Trip WHERE id=:trip_id FOR UPDATE` <br>`SELECT seats_booked FROM Tour_Booking WHERE id=:booking_id FOR UPDATE` <br>2) Cập nhật Trip: <br>`UPDATE Trip SET booked_seats = booked_seats - :seats WHERE id=:trip_id` <br>3) Xóa chi tiết và phụ thuộc: <br>`DELETE FROM Booking_Traveler WHERE booking_id=:booking_id` <br>`DELETE FROM Tour_Booking_Detail WHERE booking_id=:booking_id` <br>`DELETE FROM Invoice WHERE booking_id=:booking_id` <br>4) Xóa booking: <br>`DELETE FROM Tour_Booking WHERE id=:booking_id` <br>Commit |
| 5    |                                     | Hiển thị thông báo thành công và quay về danh sách đặt chỗ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 6    | Xem kết quả                         |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

### Luồng sự kiện phụ

- 2a. Không thỏa điều kiện xóa (đã thanh toán/không đúng trạng thái/quá hạn chốt): chặn thao tác; gợi ý dùng quy trình "Cancel Booking" (hủy đặt chỗ, hoàn tiền theo chính sách) nếu có trong phạm vi hệ thống
- 4a. Lỗi DB/transaction: rollback và báo lỗi chung

### Ràng buộc nghiệp vụ/SQL gợi ý

- Cân nhắc bật `ON DELETE CASCADE` cho các khóa ngoại, hoặc xóa theo thứ tự như ở trên để đảm bảo toàn vẹn
- Ghi log hành động xóa và lý do (audit)
